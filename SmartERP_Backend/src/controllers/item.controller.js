...
const getItems = asyncHandler(async (req, res) => {
    ...
    const { company_id } = req.params
    const result = await pool.query("select * from items where company_id=$1", [company_id])
    if (result.rows.length === 0) {
...
const getOneItem = asyncHandler(async (req, res) => {
    ...
    const result = await pool.query("select * from items where item_id=$1 and company_id=$2", [item_id, company_id])
...
export {
    createItem, getItems, getOneItem, update
};